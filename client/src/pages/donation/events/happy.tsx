...
export default function HappyEventsPage() {
  ...
  const handleEventClick = (event: any) => {
    ...
    navigate(`/donation/happy/event/${event.eventName}`);
  };
  
  // Handle create new donation event
  const handleCreateEvent = () => {
    navigate("/donation/happy");
  };
      ...
      <main className="md:ml-64 p-4 md:p-8 pt-20 md:pt-20 transition-all duration-300">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          <div>
            <h1 className="text-3xl font-bold flex items-center">
              <HandHeart className="mr-2 h-8 w-8 text-pink-600" />
              Acara Donasi Suka
            </h1>
            <p className="text-muted-foreground mt-1">
              Kelola dan lihat semua acara donasi suka
            </p>
          </div>
          <Button onClick={handleCreateEvent} className="bg-gradient-to-r from-pink-500 to-pink-700 text-white hover:opacity-90">
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle>Daftar Acara Donasi Suka</CardTitle>
          </CardHeader>
          <CardContent>
            <EventGrid
              events={events}
